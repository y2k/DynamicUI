...
sealed class Msg {
    class ItemsLoaded(val configs: List<Item>) : Msg()
...
object ConfigComponent : Component_<Model, Msg> {
    ...
    override fun init(): Cmd<Model, Msg> =
        Cmd.Effect(Model(), { Effects.loadSettings() }, Msg::ItemsLoaded)

    override fun update(state: Model, action: Msg): Cmd<Model, Msg> =
        when (action) {
            is Msg.ItemsLoaded ->
